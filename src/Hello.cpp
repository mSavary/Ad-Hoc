...
void HelloNeighborList::setLinkCode(uint8_t n) {
  mLinkCode = n;
}
...
void HelloNeighborList::setReserved(uint16_t res)  {
	mReserved = res;
}
...
void HelloNeighborList::setLinkMessageSize(uint16_t link) {
	mLinkMessageSize = link;
}
...
void HelloNeighborList::setNeighborsAddrList(short n, short o, short p, short q) {
  	IPv6* temp = new IPv6(n, o, p, q);
  	mNeighborsAddrList.push_back(*temp);
}

void HelloNeighborList::setNeighborsAddrList(IPv6* n) {
  	mNeighborsAddrList.push_back(*n);
}
