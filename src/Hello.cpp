	...
	*(send_buf + 31) = mWillingness;
	...
	for (std::list<HelloNeighborList>::iterator iv = mNeighbors.begin();
		...
		*(uint16_t*) (send_buf + c + 2) = iv->getLinkMessageSize();
		...
		for (std::list<IPv6>::iterator it = iv->getNeighborsAddrList().begin();
				it != iv->getNeighborsAddrList().end() && i < 20; it++) { // voir le nbr d'ip
			i++;
			for (int j = 0; j < 8; j++) {
				*(uint16_t*) (send_buf + c) = it->getScope(j);
				c += 2;
			}
			it = iv->getNeighborsAddrList().erase(it);
		}
		if(i==20){
			sendHello();
		} else{
			iv = mNeighbors.erase(iv);
		}
	}
...
//packetHeader
	...
	*(uint16_t*) (send_buf + 6) = mMessageSize;
	...
	receiver_endpoint = new boost::asio::ip::udp::endpoint(
			boost::asio::ip::address_v6::from_string("2014::ffff:ffff:ffff:ffff"), 7171);
	sender_endpoint = new boost::asio::ip::udp::endpoint(
			boost::asio::ip::address_v6::from_string(mOriginatorAddress->toChar()), 2050);
